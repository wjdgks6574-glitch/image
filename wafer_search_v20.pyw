...
def _list_subdirs_in_parallel(parent_dirs, log, stop_event):
    ...
    try:
        futures = {executor.submit(list_subdirs, p): p for p in parent_dirs}
        pending = set(futures)
        while pending:
...
def search_by_date_folders(wafer_id, yyyymmdd_list, log, stop_event):
    ...
    if hinted_dirs:
        ...
        log("waferID 접두사로 라인 폴더 추정: {} (시간 폴더 목록부터 조회)".format(", ".join(hint_names)))
        hour_dirs = _list_subdirs_in_parallel(hinted_dirs, log, stop_event)
        log("시간 폴더 {}개를 동시에 검색합니다...".format(len(hour_dirs)))
...
def search_sorter(wafer_id, yyyymmdd_list, log, stop_event):
    ...
    if yyyymmdd_list:
        ...
        if hinted_dirs:
            log("Sorter: waferID 접두사로 라인(BG/HM) 폴더 추정, 시간 폴더 목록부터 조회...")
            hour_dirs = _list_subdirs_in_parallel(hinted_dirs, log, stop_event)
            log("Sorter: 시간 폴더 {}개를 동시에 검색합니다...".format(len(hour_dirs)))
            found = _search_dirs_in_parallel(wafer_id, hour_dirs, log, SEARCH_WORKERS, stop_event)
            if found or stop_event.is_set():
                return found
...
def search_result_images(wafer_id, yyyymmdd_list, log, stop_event):
    ...
    if yyyymmdd_list:
        ...
        if hinted_dirs:
            log("Result_Images: waferID 접두사로 라인 폴더 추정, 시간 폴더 목록부터 조회...")
            hour_dirs = _list_subdirs_in_parallel(hinted_dirs, log, stop_event)
            log("Result_Images: 시간 폴더 {}개를 동시에 검색합니다...".format(len(hour_dirs)))
            found = _search_dirs_in_parallel(wafer_id, hour_dirs, log, SEARCH_WORKERS, stop_event)
            if found or stop_event.is_set():
                return found
...
class WaferSearchApp:
    ...
    def _search_and_copy_one(self, wafer_id):
        ...
        # (경로별 동시 검색 스레드 수는 그대로라 서버 부하는 늘지 않는다.)
        results = {}

        def run_elimages():
            if center_date:
                results["elimages"] = search_by_date_folders(
                    wafer_id, yyyymmdd_list, self.log, self.stop_event
                )
            else:
                results["elimages"] = search_full(wafer_id, self.log, self.stop_event)

        def run_sorter():
            results["sorter"] = search_sorter(wafer_id, yyyymmdd_list, self.log, self.stop_event)
