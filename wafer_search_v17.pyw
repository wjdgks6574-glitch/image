...
def date_range_folders(center_date, window_days=DATE_SEARCH_WINDOW_DAYS):
    ...
    # 그다음 전날, 다음날 순으로 검색 대상 폴더를 나열한다. 병렬 검색 시
...
def search_result_images(wafer_id, yyyymmdd_list, log, stop_event):
    ...
    if yyyymmdd_list:
        ...
        target_dirs = [
            ...
            for site_folder in RESULT_IMAGES_SITE_FOLDERS.values()
            for category in RESULT_IMAGES_CATEGORY_DIRS
            for yyyymmdd in yyyymmdd_list
        ]
...
class WaferSearchApp:
    ...
    def _search_and_copy_one(self, wafer_id):
        ...
        found += search_result_images(wafer_id, yyyymmdd_list, self.log, self.stop_event)
