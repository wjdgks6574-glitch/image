...
v9 변경사항: 날짜 인식까지는 빠른데 실제 검색이 10분 넘게 멈춰있던 문제 수정.
원인은 병렬 검색을 시작하기 전에 "라인 폴더 x 날짜" 조합마다 폴더가 실제
존재하는지 하나씩 순서대로(비병렬) 확인하는 사전 단계가 있었고, 이게 네트워크
왕복이 많아 병목이었음. 이 사전 확인 단계를 완전히 없애고(os.walk가 없는
폴더는 알아서 조용히 건너뜀), 라인 폴더 목록을 가져올 때도 os.scandir로
바꿔 폴더인지 파일인지 판단할 때 추가 네트워크 호출이 없도록 개선.
"""
