"""
Wafer ID 이미지 검색 프로그램 (v5)

\\\\172.23.11.134\\ELImages 안에서 waferID(예: ALL4260526A89264, A1L6265208320270)가
파일명에 포함된 이미지를 찾아 바탕화면\\Wafer검색결과\\{waferID}\\ 폴더로 복사합니다.

waferID에 포함된 날짜를 이용해 ELImages\\{라인}\\{YYYYMMDD}\\ 폴더 및 그 전날/다음날
(+-1일) 폴더만 검색하므로 전체 폴더를 다 뒤지지 않고 빠르게 찾습니다.

v5 변경사항: waferID 날짜 인코딩 방식을 3가지 지원하도록 확장.
  1) ALL4260526A89264 형식: "숫자6자리 + A" (기존 규칙)
  2) 앞 4자리를 제외한 다음 8자리가 전부 숫자인 경우: YYMMDDHH 그대로
     (예: ...26050404 -> 2026-05-04 04시)
  3) 앞 4자리를 제외한 다음 6자리: YY(2자리) + M(1자리 인코딩) + D(1자리 인코딩)
     + HH(2자리). 0-9는 숫자 그대로, 10 이상은 B,C,D... (A는 건너뜀)로 인코딩.
     (예: 265404 -> 2026-05-04 04시, 265B12 -> 2026-05-10 12시)
"""
...
# 1자리 인코딩: 0-9는 숫자 그대로, 10 이상은 'B'부터 시작 (A는 사용하지 않음)
_ENCODED_DIGIT_MAX_VALUE = 34  # 'Z'까지 지원
...
def search_by_date_folders(wafer_id, yyyymmdd_list, log):
    ...
    line_dirs = list_subdirs(ROOT_PATH)
    if not line_dirs:
        log("폴더 목록을 읽는 중 오류가 발생했습니다: {}".format(ROOT_PATH))
        return []
