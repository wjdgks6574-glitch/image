...
def _sorter_hinted_target_dirs(wafer_id, yyyymmdd_list):
    ...
    for hint in hints:
        ...
        if site_folder is None:
            continue
        bg_folder = "BG" + hint[2:4]
        if (site_folder, bg_folder) in seen_site_bg:
            continue
        seen_site_bg.add((site_folder, bg_folder))
        for category in SORTER_CATEGORY_DIRS:
            for yyyymmdd in yyyymmdd_list:
                dirs.append(
                    os.path.join(SORTER_ROOT, site_folder, category, yyyymmdd, bg_folder)
                )
    return dirs


def search_sorter(wafer_id, yyyymmdd_list, log):
    ...
    if yyyymmdd_list:
        ...
        if hinted_dirs:
            log("Sorter: waferID 접두사로 라인(BG) 폴더 추정, 먼저 빠르게 검색...")
            found = _search_dirs_in_parallel(wafer_id, hinted_dirs, log, SEARCH_WORKERS)
            if found:
                return found
    ...
    return _search_dirs_in_parallel(wafer_id, target_dirs, log, FULL_SEARCH_WORKERS)
...
class WaferSearchApp:
    ...
    def _search_and_copy_one(self, wafer_id):
        ...
        found += search_sorter(wafer_id, yyyymmdd_list, self.log)
