...
def _search_one_dir(wafer_id, target_dir, log, stop_event):
    if stop_event.is_set():
        # 아직 시작 전인 작업은 네트워크 호출 없이 즉시 건너뜀 (중지 시 대기
        # 중이던 나머지 폴더까지 서버에 요청을 보내는 것을 막기 위함).
        return []
...
def _search_dirs_in_parallel(wafer_id, target_dirs, log, max_workers, stop_event):
    ...
    try:
        futures = {
            executor.submit(_search_one_dir, wafer_id, target_dir, log, stop_event): target_dir
        ...
        for future in as_completed(futures):
            ...
            if stop_event.is_set() and not stopped:
                stopped = True
                log("중지 요청으로 나머지 폴더 검색을 건너뜁니다 (실행 중이던 검색은 정리될 때까지 대기)...")
    finally:
        # 대기 중인(아직 시작 안 한) 작업은 취소하되, 이미 실행 중인 스레드는
        # 끝까지 기다려서 완전히 정리한다. wait=False로 손을 떼면 백그라운드에
        # 남은 스레드가 이후 검색과 겹쳐 네트워크 요청이 계속 쌓이고, 이게
        # SMB 서버 접속 불가로 이어질 수 있었음.
...
class WaferSearchApp:
    ...
    def _search_and_copy_one(self, wafer_id):
        ...
        self.log("{}개 파일을 찾았습니다 (중지 시점까지). 복사 중...".format(len(found)))
